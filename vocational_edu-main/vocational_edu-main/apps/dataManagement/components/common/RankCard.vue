...
import SecondTitle from './Title/SecondTitle.vue'
...
interface Props {
  ...
  subtext: string // 副标题
  showIndicator?: boolean // 是否显示指标描述
  ...
  showFirstData?: boolean // 是否显示第一条数据
  chartHeight?: string | number // 图表高度
  grid?: Record<string, any> // 图表网格
}
...
<template>
  <Card :is-active="isActive">
    <div class="flex items-center justify-between">
      ...
      <Tooltip v-if="showIndicator" :content="indicatorDesc" placement="bottom">
        <template #trigger>
          <div
    ...
    <div
      v-if="showFirstData"
      class="flex justify-between items-center h-6 border-t border-[#00ffff]/20 pb-2"
    >
