...
const companyRecruitTooltipTitle = computed(() => {
  return props.companyScaleName ? `招聘${props.companyScaleName}单位总个数` : '招聘单位总个数'
})

const companyRecruitTotalPeopleTooltipTitle = computed(() => {
  return props.companyScaleName ? `${props.companyScaleName}单位招聘需求总人数` : '招聘需求总人数'
})

const companySalaryTooltipTitle = computed(() => {
  return props.companyScaleName ? `${props.companyScaleName}单位薪资中位数` : '薪资中位数'
})

const companyRecruitPositionTooltipTitle = computed(() => {
  return props.companyScaleName ? `${props.companyScaleName}单位招聘职位总个数` : '招聘职位总个数'
})
...
const tooltipFormatter = (params: any) => {
  ...
  const companyScaleNameTitle = props.companyScaleName ? props.companyScaleName : '全'
  return `
    <div class="font-bold text-gray-800 text-xs w-[300px]">
      ${params.name} ${companyScaleNameTitle + ''}规模单位招聘概览
    </div>
    <div class="flex items-center mt-2 text-xs">
      <span class="inline-block w-3 h-3 bg-gradient-to-r from-cyan-400 to-blue-400 rounded-full mr-2"></span>
      <span class="text-gray-700">${companyScaleNameTitle}规模单位招聘总个数排行：</span>
      <span class="font-bold font-DIN-Medium text-blue-600">第${item.group}名</span>
    </div>
    <div class="flex items-center mt-2 text-xs">
      <span class="inline-block w-3 h-3 bg-gradient-to-r from-cyan-400 to-blue-400 rounded-full mr-2"></span>
      <span class="text-gray-700">${companyScaleNameTitle}规模单位招聘总个数：</span>
    ...
    <div class="flex items-center mt-2 text-xs">
      <span class="inline-block w-3 h-3 bg-gradient-to-r from-cyan-400 to-blue-400 rounded-full mr-2"></span>
      <span class="text-gray-700">${companyScaleNameTitle}规模单位的招聘职位个数：</span>
      <span class="font-bold font-DIN-Medium text-blue-600">${(
        item.value ?? 0
    ...
    <div class="flex items-center mt-2 text-xs">
      <span class="inline-block w-3 h-3 bg-gradient-to-r from-cyan-400 to-blue-400 rounded-full mr-2"></span>
      <span class="text-gray-700">${companyScaleNameTitle}规模单位的招聘总人数：</span>
      <span class="font-bold font-DIN-Medium text-blue-600">${(
        item.value ?? 0
    ...
    <div class="flex items-center mt-2 text-xs">
      <span class="inline-block w-3 h-3 bg-gradient-to-r from-cyan-400 to-blue-400 rounded-full mr-2"></span>
      <span class="text-gray-700">${companyScaleNameTitle}规模单位的薪资中位数：</span>
      <span class="font-bold font-DIN-Medium text-blue-600">${(
        item.value ?? 0
    ...
    <div class="flex items-center mt-2 text-xs">
      <span class="inline-block w-3 h-3 bg-gradient-to-r from-cyan-400 to-blue-400 rounded-full mr-2"></span>
      <span class="text-gray-700">${companyScaleNameTitle}规模单位的招聘单位总个数：</span>
      <span class="font-bold font-DIN-Medium text-blue-600">${(
        item.value ?? 0
      ).toLocaleString?.()}个</span>
    </div>
  ...
  <div class="w-full h-[640px] mb-4 flex mt-4">
    <div class="w-[21%] h-full grid grid-cols-1 grid-rows-2 gap-4">
      <RankCard
        ...
        :tooltip-title="companyRecruitTooltipTitle"
        :indicator-desc="companyRecruitTooltipTitle"
        :grid="{
          top: '3%',
        }"
      />
      <RankCard
        ...
        :tooltip-title="companyRecruitTotalPeopleTooltipTitle"
        :indicator-desc="companyRecruitTotalPeopleTooltipTitle"
        :grid="{
          top: '3%',
        }"
      />
    ...
    <div class="w-[21%] h-full grid grid-cols-1 grid-rows-2 gap-4">
      <RankCard
        ...
        :tooltip-title="companySalaryTooltipTitle"
        :indicator-desc="companySalaryTooltipTitle"
        :grid="{
          top: '3%',
        }"
      />
      <RankCard
        ...
        chart-height="230px"
        :chart-data="chartData"
        :tooltip-title="companyRecruitPositionTooltipTitle"
        :indicator-desc="companyRecruitPositionTooltipTitle"
        :grid="{
          top: '3%',
        }"
      />
