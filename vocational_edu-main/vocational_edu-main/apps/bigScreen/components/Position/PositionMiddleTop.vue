...
import type { ProvinceMapData, ProfessionRecruitmentData, IndustryRecruitmentData } from '~/data/position'
...
const props = defineProps<{
  ...
  professionRecruitmentData: ProfessionRecruitmentData[]
  industryRecruitmentData: IndustryRecruitmentData[]
}>()
...
// 职业排行图的悬停处理
function handleProfessionHover() {
  // 本地已建立映射，无需请求
}
...
const highlightProvince = computed(() => {
  ...
  // 否则显示自动切换的省份
  return [...props.provinceMapData][highlightIndex.value]?.name || ''
})
...
function startHighlightLoop() {
  ...
  highlightTimer = setInterval(() => {
    highlightIndex.value = (highlightIndex.value + 1) % 34
  }, 2000)
}
...
// 基于 positionGradeList 构建：三级职业名称 -> { 一级名称, 二级名称 }
const level3ParentsMap = ref<Record<string, { level1Name: string; level2Name: string }>>({})

// 基于 industryGradeList 构建：行业名称 -> { 一级名称, 二级名称 }
...
// Tooltip 结果缓存，避免同一数据在悬停期间多次重复计算
...
function professionTooltipFormatter(params: Array<{ data?: { originalName?: string; name?: string } }>): string {
...
// 规范化地图/柱状图需要的数据结构（确保 value/group 为 number）
type BarDatum = {
  name: string
  value: number
  group: number
...
const provinceMapDataForBar = computed<BarDatum[]>(() => {
  return props.provinceMapData.map((d: ProvinceMapData) => {
    const record = d as unknown as Record<string, unknown>
    const originalName = 'originalName' in record ? (record.originalName as string | undefined) : undefined
    const totalCompanies =
      'totalCompanies' in record && typeof record.totalCompanies === 'number'
        ? (record.totalCompanies as number)
        : undefined

    return {
      name: String((record.name as string) ?? ''),
      value:
        typeof record.value === 'string' ? Number(record.value) : Number((record.value as number | undefined) ?? 0),
      group: Number((record.group as number | undefined) ?? 0),
      originalName,
      totalCompanies
    }
  })
...
type MapDatum = { name: string; value: number; rank: number }

const provinceMapDataForMap = computed<MapDatum[]>(() => {
  return props.provinceMapData.map((d: ProvinceMapData) => ({
    name: String(d?.name ?? ''),
    value: typeof d?.value === 'string' ? Number(d.value) : Number(d?.value ?? 0),
    rank: Number(d?.rank ?? 0),
    positionCount: Number(d?.positionCount ?? 0),
      ...
      <div class="h-full relative">
        <div class="flex justify-between h-full">
          <div class="w-64 grid grid-rows-2 gap-4 z-10">
            ...
            >
              <SecondTitleHeader title="招聘总人数" subtext="按省份排行">
                <template #second-svg>
              ...
              <HorizontalBarChart
                ...
                :tooltip-title="
                  props.isStatAll
                    ? props.isStatYear
                      ? `招聘需求总人数（${props.year}年）`
                      : `招聘需求总人数（${props.year}年${props.month}月）`
                    : props.isStatYear
                    ? `应届大专生招聘总人数（${props.year}年）`
                    : `应届大专生招聘总人数（${props.year}年${props.month}月）`
                "
            ...
            <div
              class="relative text-center h-full flex-shrink-0 bg-[#00ffff]/5 backdrop-blur-sm rounded-lg px-3 py-4 text-white shadow-[inset_0_0_10px_rgba(0,255,255,0.1)] border border-[#00ffff]/10 flex-1 min-h-0"
            >
              <SecondTitleHeader title="招聘总人数" subtext="按标准职业分类第三级排行">
                <template #second-svg>
              ...
              <HorizontalBarChart
                ...
                @bar-hover="handleProfessionHover"
              />
            </div>
          </div>
          <div class="w-64 flex flex-col gap-4 z-10">
            ...
            >
              <SecondTitleHeader title="招聘总人数" subtext="按标准行业分类第三级排行">
                <template #second-svg>
