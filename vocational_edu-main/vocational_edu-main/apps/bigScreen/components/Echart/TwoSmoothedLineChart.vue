...
const updateChart = () => {
  ...
  const seriesData = props.data.map((item: LineDataItem) => item.value)
  ...
  const option: EChartsOption = {
    tooltip: {
      ...
      formatter: (params: any) => {
        ...
        const title = '数据趋势'
        return `
          <div style="font-weight: bold; color: #333; margin-bottom: 8px;">${title} - ${param.name}</div>
          <div style="display: flex; align-items: center; margin: 4px 0;">
            ...
            <span style="color: #333; font-weight: bold;">${param.value}${props.yAxisUnit}</span>
          </div>
        `
      },
    },
