...
const props = withDefaults(
  defineProps<{
    ...
    quantifier?: string
    tooltipFormatter?: (_params: any, _props: any) => string
...
const formatterTooltip = (params: any) => {
  ...
  // 如果有自定义的 tooltipFormatter，则调用它并追加内容
  if (props.tooltipFormatter) {
    const customContent = props.tooltipFormatter(params, props)
...
const emit = defineEmits(['bar-hover', 'bar-mouseout', 'bar-scroll', 'bar-click'])
...
const updateChartView = (dataLength: number) => {
  ...
  // 计算当前显示的数据范围
  const actualDisplayCount = Math.min(displayCount, dataLength)
