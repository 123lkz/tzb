...
const props = defineProps<{
  ...
  tooltipTitle: string
  showCount?: number
  tooltipFormatter?: (industryName: string) => string
}>()
...
function getTooltipContent(name: string, rank: number, value: number): string {
  const baseContent = `${name}${props.tooltipTitle}\n排名: ${rank}\n数值: ${formatLargeNumber(value, 1) + props.unit}`

  if (props.tooltipFormatter) {
    const customContent = props.tooltipFormatter(name)
...
<template>
  <div class="mt-4 w-full h-full relative flex flex-col" @mouseenter="handleMouseEnter" @mouseleave="handleMouseLeave">
    <div class="flex-1 overflow-hidden">
      <Transition name="list" tag="div">
        <ListScroll :key="currentPage" :cdata="pageData" :limit="pageSize" :height="props.height || 1.65">
          <template #default="{ row, index }">
            ...
            >
              ...
              <div
                ...
                :title="getTooltipContent(row.name, currentPage * pageSize + index + 1, row.value)"
              >
                {{ row.name }}
              </div>
              <ListChart class="font-bignum whitespace-nowrap" :value="formatLargeNumber(row.value, 1) + props.unit" />
            </div>
          </template>
        </ListScroll>
      </Transition>
