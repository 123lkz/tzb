...
const props = withDefaults(
  defineProps<{
    ...
    itemStyle?: typeof defaultItemStyle
    emphasisStyle?: typeof defaultEmphasisStyle
    ...
    tooltipFormatter?: (params: any, props: any) => string
    barColor?: any
  }>(),
  {
    data: () => [],
...
const formatterTooltip = (params: any) => {
  ...
  if (typeof data.rank !== 'undefined') {
    rankHtml = `<div style="display: flex; align-items: center;">
      <span style="color: #666;">排名: </span>
      <span style="color: #333; font-weight: bold; margin-left: 4px;">${data.rank}</span>
    </div>`
  }

  return `
    <div style="font-size: 12px;">
      <div style="font-weight: bold; color: #333; margin-bottom: 8px;">${fullName}-${tooltipTitle}</div>
      ${rankHtml}
      <div style="display: flex; align-items: center;">
        <span style="color: #666;">数值: </span>
        <span style="color: #333; font-weight: bold; margin-left: 4px;">${data.value.toLocaleString()}${
    props.unit
  }${props.quantifier}</span>
      </div>
    </div>
  `
}
...
const getTooltipFormatter = () => {
  return props.tooltipFormatter
    ? (params: any) => props.tooltipFormatter(params, props)
    : formatterTooltip
}

const getBarColor = () => {
  if (props.itemStyle && props.itemStyle.color) return props.itemStyle.color
  return defaultItemStyle.color
}
...
const emit = defineEmits(['bar-hover', 'bar-mouseout'])
...
const resumeAutoScroll = (dataLength: number) => {
  if (props.data.length > displayCount) {
...
const startAutoScroll = (dataLength: number) => {
  ...
  timer = setInterval(() => {
    currentStartIndex = (currentStartIndex + 1) % dataLength
...
const updateChartView = (dataLength: number) => {
  if (!chart || dataLength <= displayCount) return

  const startPercent = (currentStartIndex / dataLength) * 100
  const endPercent = ((currentStartIndex + displayCount) / dataLength) * 450
  ...
  for (let i = 0; i < displayCount; i++) {
    ...
    const displayName =
      originalName.length > 5 ? originalName.substring(0, 3) + '...' : originalName
    ...
    currentData.push({
      ...sortedData[index],
      value: value,
      name: props.isShowProvinceData ? getProvinceSimpleName(originalName) : displayName,
      originalName,
    })
  }
...
const getCurrentYAxisData = () => {
  ...
  for (let i = 0; i < displayCount; i++) {
    ...
    if (props.isShowProvinceData) {
      displayData.push(getProvinceSimpleName(originalName))
    } else {
      displayData.push(
        originalName.length > 5 ? originalName.substring(0, 3) + '...' : originalName
      )
...
const renderChart = () => {
  ...
  const extendedData = [...sortedData, ...sortedData.slice(0, displayCount)].map(item => {
    ...
    series: [
      {
        ...
        itemStyle: props.itemStyle || defaultItemStyle,
        label: {
          show: true,
          position: 'right',
    ...
    dataZoom: [
      {
        ...
        yAxisIndex: 0,
        start: 0,
        end: (displayCount / totalLength) * 450,
        zoomLock: true, // 禁止缩放
