...
class BossSpider(BaseSpider):
    ...
    def __init__(self, headless: bool = True, browser_count: int = 1, tabs_per_browser: int = 1, city: str = "全国", 
        ...
        self.collection_name = "boss_job_raw_part2" #数据存储集合
        self.url_collection_name = f"boss_step1_urls_202504_log_part2"  # 已经爬取URL记录集合
        self.boss_urls_202504_part = "boss_step1_urls_part2"  # 需要爬取URL集合名称
    ...
    async def run(self):
        ...
        try:
            ...
            # 获取所有URL
            cursor = self.db[self.boss_urls_202504_part].find()
            self.boss_urls = await cursor.to_list(length=None)  # None表示获取所有文档
            total_jobs = len(self.boss_urls)
            logger.info(f"总共需要处理 {total_jobs} 个岗位类型")
            
            for i, url in enumerate(self.boss_urls):
                # 检查是否已经爬取过
    ...
    async def process_job_type(self, url: dict, page_index: int = 0):
        ...
        try:
            ...
            # 获取职位列表
            status = await self.get_job_list(url, page)
            if status:
                # 标记URL为已爬取
                await self.mark_url_crawled(url, success=True)
                logger.info(f"获取到超过300个岗位，该URL视为有效")
            else:
                # 标记URL为爬取失败
                await self.mark_url_crawled(url, success=False)
                logger.warning(f"岗位数量低于300，该URL视为无效: {url['url']}")                
            
        except Exception as e:
            logger.error(f"获取或处理职位列表失败: {str(e)}")
            # 标记URL为爬取失败
            # await self.mark_url_crawled(url['url'], success=False, job_count=0)
            import traceback
            logger.error(traceback.format_exc())
    ...
    async def handle_verify_code(self, page: Page) -> bool:
        ...
        try:
            ...
            if "verify-slider?callbackUrl=" in page.url:
                ...
                # 对当前页面截图保存
                image_name = f"recruitment_spider/data/bosszhipin/captcha_images/403_{int(time.time())}.png"    
    ...
    async def get_job_list(self, url: dict, page: Page) -> List[Dict]:
        ...
        for retry in range(max_retries):
            try:
                ...
                # 返回获取到的职位列表数据
                if job_list_data:
                    return job_list_data.get('zpData', {}).get('resCount', 0) > 300
    ...
    async def load_crawled_urls(self):
        ...
        try:
            ...
            async for doc in cursor:
                self.crawled_urls[f'{doc["job_type_code"]}-{doc["industry_code"]}'] = 1
    ...
    async def is_url_crawled(self, url: dict) -> bool:
        """检查URL是否已经爬取过"""
        return f"{url.get('job_type_code','')}-{url.get('industry_code','')}" in self.crawled_urls

    async def mark_url_crawled(self, url: dict, success: bool = True):
        """标记URL为已爬取"""
        try:
