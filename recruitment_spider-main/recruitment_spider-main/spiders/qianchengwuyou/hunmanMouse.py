...
class bezierTrajectory:
    ...
    def _type(self, type, x, numberList):
        ...
        # 检查分母是否接近零
        denominator = numberListre[numberListre.argmax()] - numberListre[numberListre.argmin()]
        if np.isclose(denominator, 0):
            # 如果所有元素都相同，则返回线性分布
            biaoNumberList = np.linspace(x[0], x[1], numberList)
        else:
            # 正常归一化处理
            biaoNumberList = ((numberListre - numberListre[numberListre.argmin()]) / denominator) * (x[1] - x[0]) + x[0]
